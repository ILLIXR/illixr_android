...
void native_renderer::_p_one_iteration() {
    ...
    if (!app_->is_external()) {
        ...
        auto res = vkWaitSemaphores(display_sink_->vk_device_, &wait_info, UINT64_MAX);
        if (res != VK_SUCCESS)
...
void native_renderer::create_swapchain_framebuffers() {
    ...
    for (size_t i = 0; i < display_sink_->swapchain_image_views_.size(); i++) {
        ...
        {{
         ...
         },
         {
             ...
             timewarp_->is_external()
                 ...
                 : static_cast<uint>(VK_PIPELINE_STAGE_VERTEX_SHADER_BIT | VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT),  // dstStageMask
