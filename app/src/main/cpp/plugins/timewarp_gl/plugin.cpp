...
//cl->wait_monado();
    ...
    if (!rendering_ready_) {
        ...
        for (int eye = 0; eye < 2; eye++) {
            ...
            vulkanGL_interop_buffer(image.vk_handle, image.usage);
#else
...
void timewarp_gl::vulkanGL_interop_buffer(const data_format::vk_buffer_handle& vk_buffer_handle,
    ...
    if (vk_buffer_handle.ahardware_buffer == nullptr)
...
void timewarp_gl::import_vulkan_semaphore(const semaphore_handle& vk_handle) {
        [[maybe_unused]] const bool gl_result = static_cast<bool>(eglMakeCurrent(dpy, surface_, surface_, glc));
        assert(gl_result && "glXMakeCurrent should not fail");
