...
public:
    ...
    gldemo(std::string name_, phonebook *pb_)
            : threadloop{name_, pb_}, xwin{pb->lookup_impl<xlib_gl_extended_window>()},
              sb{pb->lookup_impl<switchboard>()}, sl{pb->lookup_impl<log_service>()},
              pp{pb->lookup_impl<data_format::pose_prediction>()}, cl{pb->lookup_impl<common_lock>()},
              _m_clock{pb->lookup_impl<RelativeClock>()},
              _m_vsync{sb->get_reader<switchboard::event_wrapper<time_point>>("vsync_estimate")},
              _m_image_handle{sb->get_writer<data_format::image_handle>("image_handle")},
              _m_eyebuffer{sb->get_writer<ILLIXR::data_format::rendered_frame>("eyebuffer")} {}
    ...
    void _p_one_iteration() override {
        ...
        cl->get_lock();
        [[maybe_unused]] const bool gl_result = static_cast<bool>(eglMakeCurrent(xwin->display,
                                                                                 xwin->surface,
                                                                                 xwin->surface,
                                                                                 xwin->context));
        ...
        const data_format::fast_pose_type fast_pose = pp->get_fast_pose();
...
//            ));
        ...
        _m_eyebuffer.put(
                _m_eyebuffer.allocate<data_format::rendered_frame>(data_format::rendered_frame{
                        // Somehow, C++ won't let me construct this object if I remove the `rendered_frame{` and `}`.
                        // `allocate<rendered_frame>(...)` _should_ forward the arguments to rendered_frame's constructor, but I guess
                        // not.
                        std::array<GLuint, 2>{0, 0},
                        std::array<GLuint, 2>{which_buffer, which_buffer}, fast_pose,
                        fast_pose.predict_computed_time, lastTime}));

        which_buffer = !which_buffer;


        [[maybe_unused]] const bool gl_result_1 = static_cast<bool>(eglMakeCurrent(xwin->display,
                                                                                   NULL, NULL,
                                                                                   nullptr));
...
private:
    ...
    switchboard::writer<data_format::image_handle> _m_image_handle;
    switchboard::writer<data_format::rendered_frame> _m_eyebuffer;
...
public:
    ...
    virtual void start() override {
        cl->get_lock();
        [[maybe_unused]] const bool gl_result_0 = static_cast<bool>(eglMakeCurrent(xwin->display,
                                                                                   xwin->surface,
                                                                                   xwin->surface,
                                                                                   xwin->context));
        ...
        */
        ...
        _m_image_handle.put(_m_image_handle.allocate<data_format::image_handle>(
                data_format::image_handle{eyeTextures[0], 1,
                                          data_format::swapchain_usage::LEFT_SWAPCHAIN}));
        createSharedEyebuffer(&(eyeTextures[1]));
        _m_image_handle.put(_m_image_handle.allocate<data_format::image_handle>(
                data_format::image_handle{eyeTextures[1], 1,
        ...
        ILLIXR::math_util::projection_fov(&basicProjection, display_params::fov_x / 2.0f,
