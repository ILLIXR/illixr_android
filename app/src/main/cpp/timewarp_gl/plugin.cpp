...
typedef void (*glXSwapIntervalEXTProc)(EGLDisplay dpy, GLXDrawable drawable, int interval);
...
private:
	...
	static constexpr std::chrono::microseconds vsync_period {std::size_t(NANO_SEC/DISPLAY_REFRESH_RATE)};
	...
	GLubyte* readTextureImage(){
		...
		// Start timer
		startGetTexTime = std::chrono::system_clock::now();
		...
		// Terminate timer
		endGetTexTime = std::chrono::system_clock::now();
...
public:
	...
	virtual void _p_thread_setup() override {
		...
		eglSwapIntervalEXT(xwin->display, xwin->surface, 1);
        ...
        [[maybe_unused]] const bool gl_result_1 = static_cast<bool>(eglMakeCurrent(xwin->display, EGL_NO_SURFACE, EGL_NO_SURFACE, EGL_NO_CONTEXT));
	...
	virtual void warp([[maybe_unused]] time_type time) {
		...
		mtp_logger.log(record{mtp_record, {
			{iteration_no},
			{static_cast<std::chrono::system_clock::time_point>(time_last_swap)},
			{imu_to_display},
			{predict_to_display},
			{render_to_display},
		}});
